...
def extract_direct_stream_link(item, fid):
    # Prefer explicit direct links generated by the live session if available
    raw_link = item.get("directDownload") or item.get("link")
    if raw_link and "gofile.io" in raw_link and "/download/" in raw_link:
        return raw_link
    ...
    if server:
        # Fallback to standard node structure
