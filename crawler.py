...
def main():
  ...
  if os.path.exists("data.json"):
    try:
      ...
      print(
          f"📦 Loaded {len(existing_cache)} existing cached entries from"
          " data.json."
      )
    except Exception as e:
  ...
  for file in raw_files:
    ...
    if fid in existing_cache:
      ...
      cached_item["link"] = fresh_link
      cached_item["size"] = fresh_size
      cached_item["name"] = fname
      cached_item["edition"] = edition
      cached_item["quality"] = quality
  ...
  print(
      f"✅ Sync complete: {cached_count} existing files updated with fresh"
      f" links, {new_count} newly cataloged."
  )
