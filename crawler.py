...
def clean_title(raw_name):
    ...
    name = re.sub(r"\b(1080p|720p|2160p|4k|uhd|webrip|web-dl|bluray|x264|x265|hevc|aac)\b.*", "", name, flags=re.I)
    return name.strip()
...
def main():
    # 1. Load existing data.json cache to avoid querying Cinemeta for existing files
    existing_cache = {}
        ...
        try:
            with open("data.json", "r", encoding="utf-8") as f:
                ...
                for entry in old_entries:
                    ...
                    if fid:
                        existing_cache[fid] = entry
            print(f"📦 Loaded {len(existing_cache)} existing cached entries from data.json.")
        except Exception as e:
            print(f"⚠️ Could not parse existing data.json: {e}")
    ...
    print(f"⚡ Processing {len(raw_files)} files against local cache...")
    structured_catalog = []
    imdb_search_cache = {}
    ...
    for file in raw_files:
        fid = file["id"]
        fname = file["name"]
        fresh_link = file["link"]

        # If file was already fetched in a previous run, reuse metadata and only update link
        if fid in existing_cache:
            cached_item = existing_cache[fid].copy()
            cached_item["link"] = fresh_link  # Keep direct storage node link fresh
            cached_item["name"] = fname
            structured_catalog.append(cached_item)
            updated_files_count += 1
            continue

        # File is brand new: run detection and fetch metadata
        new_files_count += 1
    ...
    print(f"✅ Sync complete: {updated_files_count} cached items preserved, {new_files_count} new files fetched.")
