...
def extract_edition(raw_name):
  ...
  tags = []
  if "open matte" in lower or "open.matte" in lower:
    tags.append("Open Matte")
  if "imax" in lower:
    tags.append("IMAX")
  if "extended" in lower:
    tags.append("Extended Cut")
  if "director" in lower and "cut" in lower:
    tags.append("Director's Cut")
  if "theatrical" in lower:
    tags.append("Theatrical")
  if "workprint" in lower:
    tags.append("Workprint")
  if "35mm" in lower:
    tags.append("35mm Scan")
  if "remux" in lower:
    tags.append("REMUX")
  if "unrated" in lower:
    tags.append("Unrated")
...
def clean_title_string(s):
  ...
  s = re.sub(r"[\~|\-]\s*[\w\.\-]+$", "", s)
  s = re.sub(
      r"\b(msubs|subs|esub|dual audio|hindi|english|atmos|ddp5\.1|dd5\.1|5\.1|7\.1|truehd|dts\-hd|dts|aac|ac3)\b",
      "",
      s,
      flags=re.I,
  )
  s = re.sub(
      r"\b(10bit|8bit|bluray|bdrip|brrip|webrip|web\-dl|hdrip|dvdrip|remux|x265|x264|hevc|h264|h265|avc)\b",
      "",
      s,
      flags=re.I,
  )
  ...
  s = re.sub(
      r"\b(open matte|imax|extended|director\'?s cut|unrated|theatrical)\b",
      "",
      s,
      flags=re.I,
  )
...
def parse_filename(filename):
  ...
  return {"type": "movie", "title": clean_title_string(title_raw), "year": year}
...
def main():
  ...
  def process_file(fid):
    ...
    poster = (
        ...
        else "https://gofile.io/dist/img/logo-small.png"
    )
