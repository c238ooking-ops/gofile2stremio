...
def main():
    # 1. Load entire historical database into a dictionary keyed by file_id
    master_registry = {}
        ...
        try:
            with open("data.json", "r", encoding="utf-8") as f:
                ...
                for entry in old_entries:
                    ...
                    if fid:
                        master_registry[fid] = entry
            print(f"📦 Loaded {len(master_registry)} baseline entries from existing data.json.")
        except Exception as e:
            print(f"⚠️ Failed to read data.json: {e}")
    ...
    print(f"⚡ Processing {len(raw_files)} files retrieved from this scan...")
    imdb_search_cache = {}
    ...
    for file in raw_files:
        ...
        fresh_size = file["size"]
        edition = extract_edition(fname)
        quality = extract_quality(fname)

        if fid in master_registry:
            # Update existing file with fresh CDN link and details
            master_registry[fid]["link"] = fresh_link
            master_registry[fid]["size"] = fresh_size
            master_registry[fid]["name"] = fname
            master_registry[fid]["edition"] = edition
            master_registry[fid]["quality"] = quality
            updated_count += 1
            continue
    ...
    # Output the entire merged catalog
    final_output = list(master_registry.values())

    with open("data.json", "w", encoding="utf-8") as f:
        json.dump(final_output, f, indent=2)

    print(f"✅ Total catalog size: {len(final_output)} (Updated: {updated_count}, Added: {new_count}, Retained untouched: {len(final_output) - (updated_count + new_count)})")
